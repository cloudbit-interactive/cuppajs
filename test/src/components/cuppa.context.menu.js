...
export class CuppaContextMenu extends CuppaComponent {
	...
	arrow = CuppaContextMenu.ARROW.NONE;
	adjustType = CuppaContextMenu.ADJUST_TYPE.GAP;
	styleArrow = "";
	target;
	theme = "light";
	forceShow = true;
	...
	addEvents(){
		...
		cuppa.on(this.refs.dialog, 'click', (e)=>{ if (e.target === this.refs.dialog) { this.close() } }, this.groupEvents);
	...
	setPosition(){
		...
		if(!targetElement) return;
		let dimTarget = targetElement.getBoundingClientRect();
		...
		if(posX === CuppaContextMenu.POSITION.CENTER){
			dialog.style.left = `${dimTarget.x + (dimTarget.width-dim.width)*0.5}px`;
		}else if(posX === CuppaContextMenu.POSITION.LEFT){
			dialog.style.left = `${dimTarget.x - dim.width - offset}px`;
		}else if(posX === CuppaContextMenu.POSITION.LEFT_IN){
			dialog.style.left = `${dimTarget.x}px`;
		}else if(posX === CuppaContextMenu.POSITION.RIGHT){
			dialog.style.left = `${dimTarget.x + dimTarget.width + offset}px`;
		}else if(posX === CuppaContextMenu.POSITION.RIGHT_IN){
			dialog.style.left = `${dimTarget.x + dimTarget.width -  dim.width}px`;
		}
		// y
		let posY = this.posY.toUpperCase();
		if(posY === CuppaContextMenu.POSITION.CENTER){
			dialog.style.top = `${dimTarget.y + (dimTarget.height-dim.height)*0.5}px`;
		}else if(posY === CuppaContextMenu.POSITION.TOP){
			dialog.style.top = `${dimTarget.y - dim.height - offset}px`;
		}else if(posY === CuppaContextMenu.POSITION.TOP_IN){
			dialog.style.top = `${dimTarget.y}px`;
		}else if(posY === CuppaContextMenu.POSITION.BOTTOM){
			dialog.style.top = `${dimTarget.y + dimTarget.height + offset}px`;
		}else if(posY === CuppaContextMenu.POSITION.BOTTOM_IN){
			dialog.style.top = `${dimTarget.y+dimTarget.height-dim.height}px`;
		}
	}
	...
	render() {
		return html`
			<dialog ref="dialog">
				...
				`}
                <div ref="contentWrap" class="content-wrap ${this.contentClass}" style="${this.contentStyle}">
                    ${this._children}
                </div>
			</dialog>
			<style>
