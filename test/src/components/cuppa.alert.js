...
export class CuppaAlert extends CuppaComponent {
	static attributes = ["data-title", "message", "accept-text", "cancel-text", "backdrop-enabled", "input-text", "placeholder", "modal-class", "modal-style", "dialog-class", "dialog-style", 'force-show'];
	dataTitle = "Message";
	...
	modalStyle = ""
	forceShow = true;
	_disableGlobalScrollStyle;

	mounted() {
		if (this.forceShow) this.show();
		if(this.backdropEnabled) this.refs.dialog.addEventListener('click', (e)=>{ if (e.target === this.refs.dialog) { this.close() } })
	}
	...
	_disableGlobalScroll(value) {
		if (value) {
			if (this.disableGlobalScrollStyle) this._disableGlobalScrollStyle.remove();
			this._disableGlobalScrollStyle = document.createElement('style');
			this._disableGlobalScrollStyle.textContent = `
			  html, body {
			    overflow: hidden;
			    touch-action: none;
			...
			`;
			document.body.appendChild(this._disableGlobalScrollStyle);
		} else {
			if (this._disableGlobalScrollStyle) this._disableGlobalScrollStyle.remove();
		}
	}
	...
	render() {
		return html`
            <dialog ref="dialog" style="${this.dialogStyle}" class="${this.dialogClass}">
                <div ref="content" class=${`content ${this.modalClass}`} style="${this.modalStyle}" >
                    ${!this.dataTitle ? '' : html`
                        <div class="title">${html`${this.dataTitle}`}</div>`}
                    ${!this.message ? '' : html`
                        <div class="message">${html`${this.message}`}</div>`}
                    ${this.inputText == undefined && !this.placeholder ? '' : html`
                        <input
            ...
            <style>
                :root {
                    --cuppa-alert-backdrop-bg: rgba(0, 0, 0, 0.3);
                    --cuppa-alert-bg: #fff;
                    --cuppa-alert-color: #333;
                    --cuppa-alert-message-color: #666;
                    --cuppa-alert-shadow: 0px 2px 10px rgba(0, 0, 0, 0.1);
                ...
                cuppa-alert[theme=dark] {
                    ...
                    --cuppa-alert-message-color: #A1A1A1FF;
                ...
                cuppa-alert {
                    ...
                    & dialog{
                        ...
                        & .content{
                            ...
                            & .message{
                                font-size: 14px;
                                color:var(--cuppa-alert-message-color);
                            }
