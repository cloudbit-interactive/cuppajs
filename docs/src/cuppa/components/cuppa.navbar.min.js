...
export class CuppaNavBar extends CuppaComponent {
    static OPEN = "OPEN";
    static CLOSE = "CLOSE";
    ...
    status = CuppaNavBar.CLOSE;
    position = CuppaNavBar.LEFT;
    width = 300;
    ...
    mounted(){
        this.addChilds();
        if(this.status === CuppaNavBar.CLOSE){ this.close(0); }
        this.ontouchstart = this.onStart;
        //this.onmousedown = this.onStart;
    }

    initX = 0; initY = 0; difX = 0; difY = 0; posX = 0; posY = 0;
    history = [];
    onStart(e){
        this.status = CuppaNavBar.DRAGGING;
        this.internalStatus = 'checking';
        this.forceRender();
        this.initX = e.clientX || e.touches[0].clientX;
        this.initY = e.clientY || e.touches[0].clientY;
        this.posX = this.initX; this.posX = this.initY;
        this.difX = 0; this.difY = 0;
        this.history = [];
        this.setPosition(e);
    ...
    internalStatus = '';
    onMove(e){
        let {x,y} = this.setPosition(e);
        this.posX = x;
        this.posY = y;
        this.difX = x - this.initX;
        this.difY = y - this.initY;

        if(this.internalStatus === 'scrolling'){
            this.resetEvents();
            return;
        }
        if(this.internalStatus === 'checking'){
            if(Math.abs(this.difX) < 5 && Math.abs(this.difY) > 5){
                this.internalStatus = 'scrolling';
                return;
            }else if(Math.abs(this.difX) < 5){
                this.internalStatus = 'checking';
                return;
            }else{
                this.internalStatus = 'dragging';
            }
        }
        this.blockContent(true);
        this.history.push({x: x, y: y, timestamp: Date.now()});
        if(this.position === CuppaNavBar.LEFT) {
            if(this.difX > 0){ this.difX = 0; }
            if(this.difX < -this.width){ this.difX = -this.width; }
            gsap.set(this.refs.contentWrap, {left:this.difX});
            let opacity = 1 + (this.difX / this.width);
    ...
    setPosition(e){
        ...
        let position = {x: x, y: y, timestamp: Date.now()}
    ...
    onEnd(e){
        this.resetEvents();
        let swipe = this.swipeData();
        if(swipe?.isClick === true) return;
        if(this.position == CuppaNavBar.LEFT){
            if(swipe?.x < 0){
                this.close();
            }else if(swipe?.x > 0){
                ...
                if(Math.abs(this.difX) > this.width*0.5){ this.close();
                }else{ this.open(); }
            }
        }
    }
    ...
    swipeData(){
        ...
        let validHistory = this.history.filter(item => item.timestamp > timestamp - 200);
        let first = this.history[0];
        let firstValid = validHistory[0];
        if(!firstValid) return;
        let last = validHistory[validHistory.length - 1];
        if(!last) return;
        let swipe = {x: last.x - firstValid.x, y: last.y - firstValid.y, isClick:false};
        if(Math.abs(swipe.x) < 5 && Math.abs(swipe.y) < 5 && last.timestamp - first.timestamp < 400){
            swipe.isClick = true;
        }
        return swipe;
    }
    ...
    addChilds(){
        let childs = this.querySelectorAll("cuppa-navbar-content");
        if(childs.length){
    ...
    open(duration = this.durationOpen){
        this.status = CuppaNavBar.OPEN;
        if(this.tl) this.tl.kill();
        if(this.position === CuppaNavBar.LEFT) {
            this.tl = gsap.timeline();
            this.tl.set(this, { pointerEvents: 'none' });
            this.tl.to(this.refs.contentWrap, {duration, left: 0, ease: Expo.easeOut});
            this.tl.to(this.refs.blockade, {duration, opacity:1, ease: Expo.easeOut}, 0);
            this.tl.set(this, { pointerEvents: 'auto' });
        }else if(this.position === CuppaNavBar.RIGHT){
            this.tl = gsap.timeline();
            this.tl.set(this, { pointerEvents: 'none' });
            this.tl.to(this.refs.contentWrap, {duration, right: 0, ease: Expo.easeOut});
            this.tl.to(this.refs.blockade, {duration, opacity:1, ease: Expo.easeOut}, 0);
            this.tl.set(this, { pointerEvents: 'auto' });
        }
        this.forceRender();
    ...
    close(duration = this.durationClose){
        this.status = CuppaNavBar.CLOSE;
        if(this.tl) this.tl.kill();
        if(this.position === CuppaNavBar.LEFT){
            this.tl = gsap.timeline();
            this.tl.set(this, { pointerEvents: 'none' });
            this.tl.to(this.refs.contentWrap, {duration, left: -this.width, ease: Expo.easeOut});
            this.tl.to(this.refs.blockade, {duration, opacity:0, ease: Expo.easeOut}, 0);
        }else if(this.position === CuppaNavBar.RIGHT){
            this.tl = gsap.timeline();
            this.tl.set(this, { pointerEvents: 'none' });
            this.tl.to(this.refs.contentWrap, {duration, right: -this.width, ease: Expo.easeOut});
            this.tl.to(this.refs.blockade, {duration, opacity:0, ease: Expo.easeOut}, 0);
        }
        this.forceRender();
        this.blockContent(false);
    ...
    render(){
        return html`
            <div ref="blockade" class="cuppa-navbar_blockade"  @click=${()=>this.close()} ></div>
            <div ref="contentWrap" class="navbar_content_wrap"></div>
            <style>
                cuppa-navbar{ position: fixed; top: 0; bottom: 0; left: 0; right: 0; z-index: 100; overflow: auto; -webkit-user-select: none; user-select: none; }
...
customElements.define('cuppa-navbar', CuppaNavBar);
document.defaultView.CuppaNavBar = CuppaNavBar;
