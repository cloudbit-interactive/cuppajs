...
export class CuppaNavBar extends CuppaComponent {
    ...
    history = [];
    tl;
    swipeConfig = { velocityThreshold: 2, directionalOffsetThreshold: 80, gestureIsClickThreshold: 5, scrollingThreshold:10 };
    internalStatus;
    callback = null;
    updateCallback = null;

    static get observedAttributes() { return ['status', 'width', 'drag-enabled', 'backdrop-enabled', 'position'] }
    ...
    onStart(e){
        ...
        this.updateInternalStatus(CuppaNavBar.CHECKING);
        this.forceRender();
    ...
    updateInternalStatus(value){
        if(value){
            ...
            if(internalStatus === CuppaNavBar.CHECKING && Math.abs(dx) >= scrollingThreshold || Math.abs(dy) >= scrollingThreshold){
                if(Math.abs(dy) >= Math.abs(dx)*0.75){
                    this.updateInternalStatus(CuppaNavBar.SCROLLING);
                }else{
                    this.updateInternalStatus(CuppaNavBar.DRAGGING);
                }
            }
        }
    ...
    onMove(e){
        ...
        let internalStatus = this.getInternalStatus();
        if(internalStatus === CuppaNavBar.SCROLLING){ this.addExtraEvents(false); return; }
        if(internalStatus === CuppaNavBar.CHECKING){
    ...
    onEnd(e){
        ...
        if(e.target.classList.contains("cuppa-navbar_blockade") && pos){
            let first = this.getPosition(CuppaNavBar.FIRST);
            if( Math.abs(pos.dx) <= this.swipeConfig.gestureIsClickThreshold
                && Math.abs(pos.dy) <= this.swipeConfig.gestureIsClickThreshold
                && Date.now() - first.timestamp <= 500
            ){
                return;
            }
        }
    ...
    open(duration = this.durationOpen){
        ...
        if(this.position === CuppaNavBar.LEFT) {
            this.tl.to(this.refs.contentWrap, {duration, left: 0, ease: Expo.easeOut}, 0);
        }else if(this.position === CuppaNavBar.RIGHT){
            this.tl.to(this.refs.contentWrap, {duration, right: 0, ease: Expo.easeOut}, 0);
        }
    ...
    close(duration = this.durationClose){
        ...
        if(this.position === CuppaNavBar.LEFT){
            this.tl.to(this.refs.contentWrap, {duration, left: -this.width, ease: Expo.easeOut}, 0);
        }else if(this.position === CuppaNavBar.RIGHT){
            this.tl.to(this.refs.contentWrap, {duration, right: -this.width, ease: Expo.easeOut}, 0);
        }
    ...
    getPosition(position = CuppaNavBar.LAST){
        let pos = null;
        if(position == CuppaNavBar.LAST && this.history.length){
            pos = this.history[this.history.length-1];
        }else if(position == CuppaNavBar.FIRST && this.history.length){
            pos = this.history[0];
        }
        return pos;
    }
    ...
    blockContent(value = true){
        ...
        if(value === true){
            ...
            if(isMobile()) this.classList.add('noscroll');
        }else{
            this.refs.contentWrap.style.pointerEvents = '';
            this.refs.blockade.style.pointerEvents = '';
            if(isMobile()) this.classList.remove('noscroll');
        }
    }
