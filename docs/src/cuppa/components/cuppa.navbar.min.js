...
export class CuppaNavBar extends CuppaComponent {
    ...
    onMove(e){
        ...
        if(this.internalStatus === 'checking'){
            console.log(this.internalStatus, this.difX, this.difY)
    ...
    onEnd(e){
        ...
        if(this.position == CuppaNavBar.LEFT){
            if(swipe.x < 0){ this.close();
            }else if(swipe.x > 0){ this.open();
            }else{
    ...
    swipeData(){
        let validDate = new Date();
            validDate.setMilliseconds(validDate.getMilliseconds() - 100);
        let validTimestamp = validDate.getTime();
        let validHistory = this.history.filter(item => item.timestamp > validTimestamp);
        let first = validHistory[0] || {x: 0, y: 0};
        let last = validHistory[validHistory.length - 1] || {x: 0, y: 0};
        let swipe = {x: last.x - first.x, y: last.y - first.y};
        return swipe;
    }
    ...
    render(){
        return html`
            ...
            <style>
                cuppa-navbar{ position: fixed; top: 0; bottom: 0; left: 0; right: 0; z-index: 100; overflow: auto; }
