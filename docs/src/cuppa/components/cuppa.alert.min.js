...
export class CuppaAlert extends CuppaComponent {
    ...
    constructor({title = "", message = "", acceptText = "Accept", cancelText = "", backdropEnabled = false, inputText = null, placeholder = "", callback = null, closeCallback = null, className = "", topBar = false, closeImage = CuppaAlert.CLOSE_IMAGE} = {}) {
        super();
        ...
        return html`
            ...
            <div class="cuppa_alert_modal" @click=${e=>e.stopPropagation()}>
                ${!this.topBar && this.title ? 
                    html`<div class="cuppa_alert_title">${  html`${this.title}` }</div>` 
                : this.topBar ?
                    html`
                        <div class="cuppa_alert_top_bar">
                            <div class="cuppa_alert_top_bar_left">${ html`${this.title}` }</div>
                            ${!this.closeImage ? '' : html`
                                <button @click=${this.close} class="cuppa_alert_close">
                                    <img class="cuppa_alert_close_image" src="${this.closeImage}" width="14" height="14" />
                                </button>
                            `}
                        </div>
                    `
                ...
                <div ref="contentWrap" class="cuppa_alert_message">${ html`${this.message}` }</div>
            ...
            <style>
                ...
                .cuppa_alert_top_bar{ display: flex; justify-content: space-between; align-items: center; padding: 5px 5px 5px 10px;  }
                .cuppa_alert_top_bar_left{ font-size: 22px; font-weight: 700; padding: 5px; }
                .cuppa_alert_close{ transition:0.3s; width: 32px; height: 32px; cursor: pointer; display: flex; border: none; border-radius: 3px; background-color: #ff4243; justify-content: center; align-items: center; color: #fff; }
                .cuppa_alert_close:hover{ background-color: #b01516; }
                .cuppa_alert_close_image{ filter: invert(100%) sepia(0%) saturate(7487%) hue-rotate(183deg) brightness(96%) contrast(102%); }
                ...
                .cuppa_alert_modal cuppa-alert-content{ display: block; }
