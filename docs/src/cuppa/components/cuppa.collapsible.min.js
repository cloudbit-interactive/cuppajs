...
export class CuppaCollapsible extends CuppaComponent{
    ...
    animate({duration = this.duration} = {}){
        ...
        if(this.status == CuppaCollapsible.OPEN) {
            var tl = new TimelineMax({onComplete:this.onComplete});
                tl.to(this.refs.content, duration, { height:content.scrollHeight, ease:Cubic.easeInOut});
                tl.set(this.refs.content, {height:"auto"});
            if(this.refs.arrow){
                tl.to(this.refs.arrow, duration, {rotation:180, ease:Cubic.easeInOut}, 0);
            }
        }else{
            var tl = new TimelineMax({onComplete:this.onComplete});
                tl.to(this.refs.content, duration, { height:0, ease:Cubic.easeInOut});
                if(this.refs.arrow){
                    tl.to(this.refs.arrow, duration, {rotation:0, ease:Cubic.easeInOut}, 0);
                }
        }
    }
