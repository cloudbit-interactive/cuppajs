...
export class CuppaCollapsible extends CuppaComponent{
	...
	content = "";
	callback = null;
	status = this.observable("status", CuppaCollapsible.CLOSE);
	arrowURL = CuppaCollapsible.IMAGE_ARROW;
	maxHeight = 999999;
	duration = 0.5;
	...
	mounted(){
		this.addChilds();
		this.animate({duration:0});
	}

	static get observedAttributes() { return ['name', 'header', 'content', 'status', 'arrow-url', 'duration', 'group', 'ease'] }
	...
	animate({duration = this.duration} = {}){
		...
		if(typeof this.ease === "string"){ this.ease = eval(this.ease); }
		let content = this.refs.content; if(!content) return;
...
document.defaultView.CuppaCollapsible = CuppaCollapsible;
