...
export class CuppaPreviewCode extends CuppaComponent {
	...
	static get observedAttributes() { return ['mode', 'ace-theme', 'content', 'preview', 'height', 'preview-height', 'preview-width', 'disabled', 'expandable', 'show-tools-bar', 'remove-tabs', 'preview-css' ] }
	...
	configEditor(){
		...
		if(contentNode){
			...
			this.content = this.content.substring(this.content.indexOf('<!--[')+'<!--['.length);
			this.content = this.content.substring(this.content.indexOf("]-->"),-1);
			this.content = cuppa.trim(this.content);
	...
	onEditorChange(){
		...
		if(this.previewCss){
			code += `<style rel="stylesheet">${this.previewCss}</style>`;
		}
	...
	render(){
      ...
      <div ref="wrap" class="cuppa-preview-code__wrap" style="height: ${this.height}">
        ...
        ${!this.preview ? `` : html`
          <iframe ref="output" class="cuppa-preview-code__output" style="align-self: stretch;"></iframe>
        `}
      </div>
      ...
      `}
      <style>
        cuppa-preview-code{ display: flex; flex-direction: column; border: 0; border-radius: 10px; overflow: hidden; }
        ...
        .cuppa-preview-code__output{
	        ...
	        font-family: "Arial",
	        sans-serif;
	        flex:${this.previewWidth != 'auto' ? 'none' : 1};
