import {CuppaComponent} from "../../../src/cuppa.component.min.js"
import {cuppa} from "../../../src/cuppa.min.js"
import Todo from "./Todo.js"

export default class TodoItem extends CuppaComponent {
    cuppa = cuppa
    state = {item:null, rename:false, index:null}

    static get observedAttributes() { return ['item', 'index'] }
    attributeChangedCallback(attr, oldVal, newVal) { this.setState({[attr]:cuppa.jsonDecode(newVal)}) }

    onRename(e){
        let todoList = this.getData(Todo.STORE_TODO, {store:"local"})
            todoList[this.state.index].name = e.target.value;
        this.setData(Todo.STORE_TODO, {data:todoList, store:"local"})
        this.setState({rename:false});
    }

    onDone(){
        let todoList = this.getData(Todo.STORE_TODO, {store:"local"})
            todoList[this.state.index].done = !this.state.item.done
        this.setData(Todo.STORE_TODO, {data:todoList, store:"local"})
    }

    onDelete(){
        let todoList = this.getData(Todo.STORE_TODO, {store:"local"})
            todoList.splice(this.state.index, 1)
        this.setData(Todo.STORE_TODO, {data:todoList, store:"local"})
    }
    
    render(){
        return /*html*/`
            <li>
               <span style="text-decoration:${(this.state.item?.done) ? 'line-through' : 'none' }; display:${!this.state.rename ? "auto" : "none"} ">${ this.state.item?.name || "" }</span>
               <input value="${this.state.item?.name}" onchange="this.onRename" style="display:${this.state.rename ? "auto" : "none"}" />
               <button onclick="()=>this.setState({rename:true})" style="display:${this.state.rename ? 'none' : 'inline-block'}" >Rename</button>
               <button onclick="this.onDone">${this.state.item?.done ? 'Undone' : 'Done' }</button>
               <button onclick="this.onDelete">Delete</button>
            </li>`
    }
}
customElements.define('todo-item', TodoItem);