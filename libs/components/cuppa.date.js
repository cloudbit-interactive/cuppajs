...
export class CuppaDate extends CuppaComponent {
	static monthTypes = {NUMBER:'NUMBER', NAME:'NAME', SHORT_NAME:'SHORT_NAME', NUMBER_AND_NAME:'NUMBER_AND_NAME', NUMBER_AND_SHORT_NAME:'NUMBER_AND_SHORT_NAME'}
	currentDate = new Date();
	value = this.observable('value');
	minYear = this.observable('minYear', this.currentDate.getFullYear()-50);
	maxYear = this.observable('maxYear', this.currentDate.getFullYear()+50);
	showYear = this.observable('showYear', true);
	showMonth = this.observable('showMonth', true);
	monthType = this.observable('monthType', CuppaDate.monthTypes.SHORT_NAME);
	showDay = this.observable('showDay', true);
	callback;

	static get observedAttributes() { return ['value', 'min-year', 'max-year', 'show-year', 'show-month', 'show-day', 'month-type'] }
	...
	onChange(){
		const year = this.refs.yearSelector?.value || this.currentDate.getFullYear();
		const month = parseInt(this.refs.monthSelector?.value)-1 || this.currentDate.getMonth();
		const day = this.refs.daySelector?.value || this.currentDate.getDate();
		const hour = this.refs.hourSelector?.value || this.currentDate.getHours();
		const minute = this.refs.minuteSelect?.value || this.currentDate.getMinutes();
		const second = this.refs.secondSelect?.value || this.currentDate.getSeconds();
	...
	getDate(){
		...
		if(value.length <= 4) value = value + "-01";
		if(value.length <= 7) value = value + "-01";
		if(value.length <= 10) value = value + " 00";
		if(value.length <= 13) value = value + ":00";
	...
	render(){
		...
		let lastDay = getLatMonthDay(date.getMonth()+1, date.getFullYear());
		return html`
			${!this.showYear ? `` : html`
				<select class="cuppa-date__year" ref="yearSelector" @change=${this.onChange}>
					${arrayFromTo(this.minYear, this.maxYear).map(year => {
						return html`<option value="${year}" ?selected=${year==date.getFullYear()} >${year}</option>`;
					})}
				</select>
			`}
			${!this.showMonth ? `` : html`
				<select class="cuppa-date__month" ref="monthSelector" @change=${this.onChange}>
					${arrayFromTo(1, 12).map(month => {
						let label = [];
						if(this.monthType.indexOf(CuppaDate.monthTypes.NUMBER) !== -1) {
							label.push(String(month).padStart(2,"0"));
						}
						if(this.monthType.indexOf(CuppaDate.monthTypes.SHORT_NAME) !== -1){
							label.push(monthToString(month-1, true));
						}else if(this.monthType.indexOf(CuppaDate.monthTypes.NAME) !== -1){
							label.push(monthToString(month-1, false));
						}
						return html`<option value="${month}" ?selected=${month==date.getMonth()+1} >${label.join(' - ')}</option>`;
					})}
				</select>
			`}
			${!this.showDay ? `` : html`
				<select class="cuppa-date__month" ref="daySelector" @change=${this.onChange}>
					${arrayFromTo(1, lastDay).map(day => {
						return html`<option value="${day}" ?selected=${day==date.getDate()} >${(String(day)).padStart(2,"0")}</option>`;
					})}
				</select>
			`}
			<style>
				cuppa-date{ display: inline-flex; justify-content: flex-start; align-items: center; gap:0.2rem;}
				cuppa-date select{
...
customElements.define('cuppa-date', CuppaDate);

/* ArrayFromTo */
let arrayFromTo = function(from, to, fillWith){
...
let getLatMonthDay = function(month = 1, year = 2008, {returnString = true} = {}){
	month = month-1;
	let day = new Date(year, month+1, 0).getDate();
	if(returnString) return left0(day);
	return day;
}
...
let monthToString = function(month, short = true){
	month = parseInt(month);
	return (short) ? monthsShort[month] : monthsLong[month];
}
