...
export class CuppaContextMenu extends CuppaComponent {
	...
	static attributes = ['target', 'pos-x', 'pos-y', 'force-show', 'force-remove', 'arrow', 'style-arrow', 'theme', 'show-on-mouse-over', 'close-on-scroll', 'main-menu-class', 'main-menu-style'];
	static observables = ['posX', 'posY', 'arrow'];
	posX = CuppaContextMenu.POSITION.RIGHT;
	posY = CuppaContextMenu.POSITION.TOP_IN;
	arrow = CuppaContextMenu.ARROW.UP;
	styleArrow = "";
	target;
	theme = "light";
	forceShow = false;
	...
	processSubMenus(){
		...
		for(const submenu of subMenus){
			...
			cuppa.on(targetElement, `mouseenter`, (e)=>{
				...
				this.setPosition({targetElement, menu:submenu});
			}, this.groupEvents);
			cuppa.on(targetElement, `mouseleave`, (e)=>{
				clearTimeout(targetElement.delayCloseTimeout);
				targetElement.delayCloseTimeout = setTimeout(()=>{
					if(!submenu.onMenu){
						submenu.style.display = "none";
					}
				}, this.subMenuCloseDelay);
			}, this.groupEvents);
			cuppa.on(submenu, `mouseenter`, (e)=>{
				submenu.onMenu = true;
			}, this.groupEvents);
			cuppa.on(submenu, `mouseleave`, (e)=>{
				submenu.onMenu = false;
				submenu.style.display = "none";
			}, this.groupEvents);
		}
	}
	...
	render() {
		return html`
            <dialog ref="dialog">
                <div ref="mainMenu" class="main-menu ${this.mainMenuClass}" style="${this.mainMenuStyle}">
                    ${this.arrow === "" || this.arrow.toUpperCase() === CuppaContextMenu.ARROW.NONE ? `` : html`
	                    <svg class="cuppa-arrow ${this.arrow.toLowerCase()}" width="10" height="5" viewBox="0 0 18 9" preserveAspectRatio="none" style="${this.styleArrow}" >
            ...
            <style>
                ...
                cuppa-context-menu, cuppa-context-menu * {
                ...
                cuppa-context-menu{
                    & dialog{
                        ...
                        & .main-menu, cuppa-context-sub-menu{
                            ...
                            & .cuppa-arrow{
	                            position: absolute; fill:var(--cuppa-context-menu-arrow-color);
                                &.up{ top:-4px; left:15px; }
                                &.left{ transform: rotate(-90deg); top:15px; left:-7px; }
                                &.right{ transform: rotate(90deg); top:15px; right:-7px; }
                                &.down{ transform: rotate(180deg); bottom: -4px; left:15px; }
                            }
                            ...
                            & button{
                                ...
                                & hr{ border:none; border-bottom: var(--cuppa-context-menu-separator); margin:7px 0; }
                                & .shortcut{ display: flex; align-items: center; justify-content: flex-end; color:var(--cuppa-context-menu-shortcut-color); min-width: 60px; padding-left: 20px; }
                                & img{ height: 14px; margin-right: 14px; filter: var(--cuppa-context-menu-tint); }
                                & i{ width: 14px; height: 14px; background: no-repeat center; background-size: contain; }
                                & i{ margin: 0 14px 0 0;
                                	&.more{
                                        width: 12px;
                                        height: 12px;
                                        filter: var(--cuppa-context-menu-tint);
                                        margin:0 0 0 14px;
...
customElements.define('cuppa-context-menu', CuppaContextMenu);
document.defaultView.CuppaContextMenu = CuppaContextMenu;
