...
export class CuppaSelect extends CuppaComponent {
	...
	constructor() {
		super();
		this.observables({placeholder:this.placeholder, selected:this.selected, selectClass:this.selectClass, options:this.options, search:this.search, open:this.open, dropdownStyle:this.dropdownStyle});
	}

	static get observedAttributes() { return ['placeholder', 'type', 'select-class', 'name', 'selected', 'open', 'dropdown-style'] }
	...
	getSelectedLabel(){
		...
		let opt = this.options.filter(opt=>opt[this.valueColumn] == this.selected)[0];
		let label = opt;
		if (typeof opt === "object") {
			label = this.labelColumn.split(',').map(labelColumn => opt[labelColumn.trim()]);
			label = label.join(this.labelSeparator);
		}
	...
	render(){
		const value = this.getSelectedLabel();
		let options = (this.options || []).filter(opt=>String(opt[this.labelColumn]).toLowerCase().indexOf(this.search.toLowerCase()) !== -1);
		return html`
			<div 
				ref="select"
