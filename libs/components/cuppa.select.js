...
export class CuppaSelect extends CuppaComponent {
	static type = {SINGLE: 'SINGLE', MULTIPLE: "MULTIPLE"};
	static attributes = [
		'placeholder',
		'type',
		'select-class',
	...
	show() {
		...
		if(this.closeOnScroll) window.addEventListener('scroll', this.onScroll);
	}
	...
	selectValue(value, {close = true} = {}) {
		if (this.type === CuppaSelect.type.MULTIPLE) {
	...
	render() {
		...
		return html`
            ...
            >
                ...
                ${this.renderSelectedItem ? this.renderSelectedItem(this.selected, this) : html`
                    <span
                        class="${this.nowrap ? `nowrap` : ''}"
                ...
                ${!this.showArrow ? `` : html`<img class="arrow-img" src="${arrowImage}" alt="arrow"/>`}
            </div>
			...
			this.selectValue('');
                            ...
                            >
                                <span class="${this.nowrap ? `nowrap` : ''}">${this.emptyOptionLabel}</span>
                            </div>
                        `}
			...
			if(this.renderItemOnList){
				return this.renderItemOnList(opt, index, this)
			}else{
				return html`
                                    <div
                                        class=${`list-item`}
