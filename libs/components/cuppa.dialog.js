...
export class CuppaDialog extends CuppaComponent {
	static attributes = ['target', 'force-show', 'close-on-backdrop', 'content-class', 'content-style', 'dialog-class', 'dialog-style', 'theme', 'disable-global-scroll'];
	static observables = ['content'];
	target = '';
	targetElement;
	groupEvents = `CuppaDialog_${crypto.randomUUID()}`;
	forceShow = false;
	...
	close() {
		...
		setTimeout(() => {this.refs.dialog.close() }, 200)
	}
		...
		return html`
            ...
            <style>
                ...
                cuppa-dialog[theme=dark] {
                    ...
                    --cuppa-dialog-bg: #333;
                    --cuppa-dialog-shadow: 0px 3px 10px rgba(0, 0, 0, 0.1);
                    --cuppa-dialog-color: #FFF;
                }

                cuppa-dialog, cuppa-dialog * {
