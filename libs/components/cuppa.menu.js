...
export class CuppaMenu extends CuppaComponent {
	...
	static get observedAttributes() { return ['target', 'pos-x', 'pos-y', 'force-show', 'force-remove'] }
	...
	setPosition(){
		...
		if(posX === CuppaMenu.POSITION.CENTER){
			...
			this.style.left = `${dimTarget.x - dim.width}px`;
		...
		if(posY === CuppaMenu.POSITION.CENTER){
			...
			this.style.top = `${dimTarget.y-dim.height}px`;
