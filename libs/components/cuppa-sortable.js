...
export class CuppaSortable extends CuppaComponent {
	static attributes = ['handler', 'on-drag-class', 'drag-opacity', 'return-value', 'value']
	handler;
	dropCallback;
	onDragClass = 'move-right';
	dragOpacity = 0.5;
	value;
	returnValue = false;
	...
	configHandler(){
		const handler = this.querySelector(this.handler);
		if(handler){
			handler.onmousedown = (e) => {
				this.draggable = true;
			}
			handler.onmouseleave = (e) => {
				cuppa.setAttribute('cuppa-sortable', 'draggable', 'false');
			}
		}
	}
	...
	onDragStart(e) {
		...
		cuppa.addClass('cuppa-sortable', 'd-i-children');
		this.classList.add('d-i-children');
		e.target.style.opacity = this.dragOpacity;
		this.dispatchEvent(new CustomEvent("start", {detail:this}));
	}
	
	onDragEnd(e){
		this.classList.remove('drag-element');
		cuppa.removeClass('cuppa-sortable', 'd-i-children');
		cuppa.removeClass('cuppa-sortable', this.onDragClass);
	...
	onDragEnter(e){
		...
		this.classList.add(this.onDragClass);
		this.dispatchEvent(new CustomEvent("enter", {detail:this}));
	}
	
	onDragLeave(e){
		this.classList.remove(this.onDragClass);
